...
public class SwaggerConfig {
    ...
    public OpenAPI apiInfo() {
        Info apiInfo = new Info()
                ...
                .description("APIs for review-service. Данный сервис завязан вокруг уведомлений и служит для взаимодействия остальной части приложения с уведомлениями")
                .version("1.0.0");
        Components components = new Components().addSecuritySchemes(OAUTH_SCHEME_NAME, createOAuthScheme());
        final var localServer = new Server().description("Локальное окружение").url("http://localhost:9494");
        final var devServer = new Server().description("ДЕВ окружение").url("https://api.migration.it-mentor.space/notification-service");
