...
public class TransactionOutcomePendingHandler extends TransactionOutcomeHandler {
    ...
    public boolean isHandle(TransactionEvent event) {
        return super.isHandle(event) && event.status() == TransactionStatus.PENDING;
    }
    ...
    public void handle(TransactionEvent event) {
        try {
            ...
            Notification notification = notificationMapper.transactionOutcomePendingToNotification(event, user);
