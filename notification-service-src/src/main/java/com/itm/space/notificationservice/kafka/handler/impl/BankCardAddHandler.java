...
public class BankCardAddHandler implements BankCardHandler<BankCardEvent> {
    ...
    public boolean isHandle(BankCardEvent event) {
        return event.type() == BankCardEventType.ADD;
    }
    ...
    public void handle(BankCardEvent event) {
        try {
            ...
            Notification notification = notificationMapper.newMessageBankCardWhereAdd(event, user);
