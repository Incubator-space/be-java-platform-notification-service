...
public class TransactionIncomeWaitingHandler extends TransactionIncomeHandler {
    ...
    public boolean isHandle(TransactionEvent event) {
        return super.isHandle(event) && event.status() == TransactionStatus.WAITING_FOR_USER;
    }
    ...
    public void handle(TransactionEvent event) {
        try {
            ...
            Notification notification = notificationMapper.toEntity(event, user);
